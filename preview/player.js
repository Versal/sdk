    ...
    errorHandler: function(model, resp, options) {
      ...
      if (model instanceof Error) {
        ...
        switch (resp.status) {
          case 401:
          case 403:
            return new ApiErrors.PermissionDenied;
          case 404:
            return new ApiErrors.NotFound;
          default:
            return new ApiErrors.ApplicationError;
        }
      } else {
...
(function() {
  ...
  define('messages/handlers/style_register',[], function() {
    var handler,
      _this = this;
    ...
    handler.namespaceRule = function(key, rule) {
      var content, offset, selectors, token;
      rule = rule.trim();
      offset = rule.indexOf('{');
      switch (token = rule.substr(0, rule.indexOf(' '))) {
        case '@media':
          content = rule.substring(offset + 1, rule.lastIndexOf('}') - 1);
          return rule.substring(0, offset + 1) + handler.namespaceCss(key, content);
        case '@keyframe':
          return rule;
        default:
          selectors = [];
          _.each(rule.substr(0, offset).split(','), function(selector) {
            selector = handler.namespaceSelector(key, selector);
            if (selector) {
              return selectors.push(selector);
            }
          });
          return selectors.join(',\n') + ' { ' + rule.substr(offset + 1);
      }
    };
    handler.namespaceCss = function(key, data) {
      var depth, i, offset, result, rule, rules;
      result = [];
      rules = data.trim().split('}');
      i = -1;
      while (rule = rules[++i]) {
        offset = rule.indexOf('{');
        if (rule.lastIndexOf('{') > offset) {
          depth = rule.split('{').length - 1;
          while (depth > 0 && ++i < rules.length) {
            depth -= 2 - rules[i].split('{').length;
            rule += '}' + rules[i];
          }
        }
        result.push(handler.namespaceRule(key, rule));
      }
      return result.join(' }\n') + ' }';
    };
    handler.addStyle = function(data, key) {
      var $style, cssClass;
      cssClass = "style-" + key;
...
(function() {
  ...
  define('app/mediator',['messages/mediator', 'messages/handlers/asset_select', 'messages/handlers/style_register'], function(Mediator, assetSelectHandler, styleRegisterHandler) {
    ...
    return GadgetInstanceView = (function(_super) {
      ...
      GadgetInstanceView.prototype.instantiateGadget = function(klass, defaultConfig) {
        ...
        try {
          ...
          new klass(options, options.config.toJSON(), options.$el);
          ...
          return typeof console !== "undefined" && console !== null ? typeof console.error === "function" ? console.error('Gadget error: ' + e, e.stack) : void 0 : void 0;
        }
      };
...
(function() {
  ...
  define('app/catalogue',['collections/gadget_catalogue', 'gadgets/section/gadget'], function(GadgetCatalogue, SectionGadget) {
    ...
    localGadgets = new GadgetCatalogue([
      {
        ...
        hidden: true,
        icon: {
          "default": "http://placekitten.com/100/100"
        },
...
(function() {
  ...
  define('views/lesson',['cdn.marionette', 'app/mediator', 'views/gadget_instance', 'views/section', 'app/catalogue', 'plugins/vs-sticky', 'cdn.jqueryui'], function(Marionette, mediator, GadgetInstanceView, SectionView, gadgetCatalogue, VsSticky) {
    var Lesson;
    _.extend(vs.api.Gadget.prototype, {
      ...
      resolve: function(opts) {
        ...
        if (!this.gadgetProject) {
          return this.onResolveError('Unknown project');
        }
    ...
    return Lesson = (function(_super) {
      ...
      Lesson.prototype.appendHtml = function(collectionView, itemView, index) {
        ...
        return itemView.on('gadgetRendered', function() {
          var renderedAll, renderedCount;
          itemView.rendered = true;
          renderedCount = (_this.children.filter(function(i) {
            return i.rendered;
          })).length;
          renderedAll = renderedCount >= _this.children.length;
          if (renderedAll) {
            _this.stickHeaders();
          }
