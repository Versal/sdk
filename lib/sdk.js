...
sdk.configuration = {
  // TODO maybe this can be one value after frontdoor goes away¿
  rootUrl: "http://stack.versal.com"
  apiPath: "api2"
};

_.extend(sdk.configuration, {
  gadgetUploadUrl: sdk.configuration.rootUrl + "/" + sdk.configuration.apiPath + "/gadgets",
  frontdoorUrl: sdk.configuration.rootUrl + "/" + "/frontdoor/signin",
  verifySessionUrl: sdk.configuration.rootUrl + "/" + sdk.configuration.apiPath + "/user",
...
sdk.authorize = function(options, callback){
  ...
  sdk.verifySession(null, function(err, sessionOK){
    if(sessionOK) { 
      callback.call(this); 
    } else {
...
sdk.verifySession = function(options, callback) {
  options = options || {};

  var needle = require('needle'),
      session_id = options.session_id || sdk.getSessionId(),
      callback = callback || sdk.getLogger("Your session is OK!");

  if(!session_id) {
    return callback.call(this, new Error("SESSION_ID has not been found in ~/.versal. Provide your session in --session_id option."));
  }

  needle.get(sdk.configuration.verifySessionUrl, { headers: { "SESSION_ID": session_id }}, function(err, res, body) {
