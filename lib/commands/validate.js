...
var validate = module.exports = {
  manifestFields: [
    { name: "name", required: true, regex: /^[A-Za-z0-9-_]{2,}$/, message: "Minfest.json: name can contain only latin letters, numbers, dashes and underscores." },
...
validate.validateFolder = function(gadgetPath) {
  ...
  var errors = validate.validateManifest(manifest);

  var files = glob.sync("**", { cwd: gadgetPath });
  errors = errors.concat(validate.validateFiles(files));

  return errors;
};

validate.validateFiles = function(files) {
  var required = validate.requiredFiles;

  var missing = _.difference(required, files);
  return _.map(missing, function(file){ file + " is required, but not present in the gadget folder"; });
};

validate.validateManifest = function(manifest) {
  var fields = validate.manifestFields;
  var errors = [];
