...
var rootUrl = "http://testbeta.versal.com";
...
sdk.validate = function(options, callback){
  ...
  var errors = validator.validateFolder(compiledPath);
...
sdk.compress = function(options, callback) {
  ...
  glob("**/*.*", { cwd: distPath }, function(err, files){
    ...
    archive.finalize(function(err) { callback.call(this, null, options); });
  });
}
...
sdk.upload = function(options, callback) {
  ...
  needle.post(uploadUrl, { content: { buffer: fileData, filename: "bundle.zip", content_type: 'application/zip' }}, { multipart: true, headers: { "session_id": session_id } }, function(err, res){
    if(err) {
      ...
      callback.call(this, new Error("Publishing unsuccessful. Error code: " + res.statusCode));
    } else {
...
sdk.saveSessionId = function(options) {
  ...
  fs.writeFile(keyPath, session_id, function(err){
    if(err) throw err;
...
sdk.alive = function(options, callback) {
  ...
  var needle = require('needle'),
      session_id = options.session_id || sdk.getSessionId(),
      callback = callback || function(err, alive) { if(err) throw err; console.log(alive ? "Your session is alive".green : "Your session is dead".red); }

  needle.get(sdk.configuration.aliveUrl, { headers: { "SESSION_ID": session_id }}, function(err,res) {
    callback.call(this, err, res && res.statusCode === 200);
  });
}
...
sdk.signin = function(options, callback) {
  ...
  needle.post(sdk.configuration.frontdoorUrl, data, function(err, res) {
    if(err) {
      ...
      callback.call(this, null, session_id);
    } else {
      callback.call(this, new Error("Access denied. Code: " + res.statusCode));
    }
  });
}