...
define('models/gadget_project',['require','exports','module','underscore','backbone','jquery','./gadget'],function (require, exports, module) {(function() {
  var $, Backbone, Gadget, GadgetProject, _,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  module.exports = GadgetProject = (function(_super) {
    ...
    GadgetProject.cache = {
      get: function(type) {
        return GadgetProject.cache[type];
      },
      set: function(type, klass) {
        return GadgetProject.cache[type] = klass;
      }
    };
    ...
    GadgetProject.prototype.path = function(file) {
      var base, segments;
      base = GadgetProject.prototype.apiUrl || '';
      segments = [base, this.urlRoot].concat(this.typeSegments());
      if (file) {
        segments.push(file);
      }
    ...
    GadgetProject.prototype.typeSegments = function() {
      return [this.get('username'), this.get('name'), this.get('version')];
    };
    ...
    GadgetProject.prototype.changeCatalog = function(catalog, options) {
      if (options == null) {
        options = {};
      }
      if (!catalog) {
      ...
      options.url = "/gadgets/" + this.id + "/status";
      return this.sync('create', this, options);
    };
    ...
    GadgetProject.prototype.validate = function(attrs) {
      if (!attrs.username) {
        return 'username is required for gadget project';
      }
      if (!attrs.name) {
        return 'name is required for gadget project';
      }
      if (!attrs.version) {
        return 'version is required for gadget project';
      }
    };

    GadgetProject.prototype.fetchConstructor = function(options) {
      var error, klass, promise, success,
        _this = this;
      if (options == null) {
        options = {};
      }
      if (!this.has('classDefinition')) {
        if (!GadgetProject.prototype.apiUrl) {
          throw new Error('apiUrl is required to resolve a gadget. Did you call api.init?');
        }
        if (!GadgetProject.prototype.requirejs) {
          throw new Error('requirejs has to be provided to resolve a gadget. Did you call api.init?');
        }
      }
      success = function(klass) {
        if (!_this.has('classDefinition')) {
          _this.set('classDefinition', klass);
          if (!GadgetProject.cache.get(_this.type)) {
            GadgetProject.cache.set(_this.type(), klass);
          }
        }
        promise.resolve(klass);
        if (options.success) {
          options.success.apply(_this, [klass]);
        }
        return _this.trigger('resolve:success', klass);
      };
      error = function(err) {
        promise.reject(err);
        if (options.error) {
          options.error.apply(_this, [err]);
        }
        return _this.trigger('resolve:error', err);
      };
      promise = $.Deferred();
      if (klass = this.get('classDefinition')) {
        success(klass);
      } else if (klass = GadgetProject.cache.get(this.type())) {
        success(klass);
      } else {
        GadgetProject.prototype.requirejs(["" + GadgetProject.prototype.apiUrl + (this.main())], success, error);
      }
