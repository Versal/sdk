...
module.exports = (function (sdk, config) {
  ...
  function lessonGadgets (gadgets) {
  ...
  id = 0
  function gadget (gadgetPath) {
    var gadget, manifest, manifestPath;
    if (gadgetPath) { // gadget is compiled and we know it's path
      manifestPath = path.join(gadgetPath, 'manifest.json');
      if (!fs.existsSync(manifestPath)) {
        console.log('Gadget at '+gadgetPath+' not found, did you run versal compile for it?');
        return null;
      }
